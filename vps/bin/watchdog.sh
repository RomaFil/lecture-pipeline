...
if [ "$(date +%-H)" -ge "$DIGEST_HOUR" ] && [ "$last_digest" != "$today" ]; then
    ...
    # Посилання «той самий випадок, що й таймліміт вище» має сенс лише тоді, коли
    # рядок про таймліміт справді є у звіті (21.09.2026 звіт відсилав до рядка,
    # якого не було, — 1 невдалий запуск без жодного таймліміту).
    if [ "$fa" -gt 0 ]; then
        if [ "$to" -gt 0 ]; then
            attention+=("— Невдалих запусків за добу: $fa — здебільшого той самий випадок, що й таймліміт вище")
        else
            attention+=("— Невдалих запусків за добу: $fa — задача синка завершилась із помилкою без таймліміту. Найчастіше VPN не піднявся після пробудження ПК (у sync-логу ERROR «VPS недоступний»); наступний запуск зазвичай відпрацьовує сам. Якщо в черзі на ПК не 0 — дивись хвіст sync-логу")
        fi
    fi
    ...
    lines=(
        ...
        "Черга: $n_in на обробці, $n_out готових чекають на ПК, $n_rev незрозумілих, $n_arch в архіві"
        "Диск VPS: $disk"
